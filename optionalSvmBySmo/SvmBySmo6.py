...
class BinarySMO_SVM:
    ...
    def __init__(self, C=1.0, kernel='rbf', gamma=0.5,
                 tol=1e-3, max_passes=3, max_epochs=25, random_state=0):
        """
        :param C: 惩罚系数
...
if __name__ == "__main__":
    ...
    # 使用 krkopt 数据集的随机子集进行训练。
    N_SUBSET = 12000  # 子采样设置：在完整 28057 个样本中随机取 12000
    if N_SUBSET < n_total:
    ...
    feature_std = X.std(axis=0, keepdims=True) + 1e-8
