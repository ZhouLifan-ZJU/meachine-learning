import numpy as np
from NN import apply_dropout_backward  # 引入 Dropout 反向方法
...
def nn_backward(nn, batch_y):
    ...
    if nn.batch_normalization:
        ...
        nn.delta[nn.depth - 1] = nn.Gamma[nn.depth - 2] * nn.delta[nn.depth - 1] / np.tile(
            (nn.S[nn.depth - 2] + 0.0001), (1, m))

    nn.W_grad[nn.depth - 2] = np.dot(nn.delta[nn.depth - 1], nn.a[nn.depth - 2].T) / m + nn.weight_decay * nn.W[
        nn.depth - 2]
    ...
    for i in range(1, nn.depth - 1):
        ...
        # ⭐ Dropout backward
        nn.delta[k] = apply_dropout_backward(nn, k, nn.delta[k])
